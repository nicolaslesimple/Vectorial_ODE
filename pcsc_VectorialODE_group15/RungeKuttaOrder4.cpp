 ...
 */
RungeKuttaOrder4::RungeKuttaOrder4(Input &input, Solution& solution) : RungeKutta(input, solution) {}
...
void RungeKuttaOrder4::SolveVectorialODE() {
    ...
    for (int i = 1; i < overall_timestep; ++i) { // Solve the system for a fixed number of time steps. Runge-Kutta method needs the timestep i-1 to interpolate so this method will integrate it
        kutta1 = AddTwoVector(MultiplyWithVectorByRight(input->GetCoefficientMatrix(),tmp1), GetColumnsOfMatrix(input->GetFunctionMatrix(), i));
        tmp2 = GetColumnsOfMatrix(input->GetFunctionMatrix(), 1 + i);
        store = MultiplyVectorAndScalar(AddTwoVector(tmp2, GetColumnsOfMatrix(input->GetFunctionMatrix(), i)),0.5); //Interpolating
        kutta2 = AddTwoVector(MultiplyWithVectorByRight((input->GetCoefficientMatrix()), AddTwoVector(tmp1, MultiplyVectorAndScalar(kutta1, (timestep / 2)))), store);
        kutta3 = AddTwoVector(MultiplyWithVectorByRight((input->GetCoefficientMatrix()), AddTwoVector(tmp1, MultiplyVectorAndScalar(kutta2, (timestep / 2)))), store);
        kutta4 = AddTwoVector(MultiplyWithVectorByRight((input->GetCoefficientMatrix()), AddTwoVector(tmp1, MultiplyVectorAndScalar(kutta3, (timestep)))), tmp2);

        vector<double> kutta_coefficents = AddTwoVector(AddTwoVector(AddTwoVector(kutta1, MultiplyVectorAndScalar(kutta2,2)), MultiplyVectorAndScalar(kutta3,2)),kutta4);
        tmp1 = AddTwoVector(tmp1, MultiplyVectorAndScalar(kutta_coefficents, (timestep/6))); //Apply the definition of the Runge-Kutta method 1 step : https://fr.wikipedia.org/wiki/M%C3%A9thodes_de_Runge-Kutta
